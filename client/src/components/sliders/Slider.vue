...
onMounted(() => 
  slider.wrapperPaddingLeft = 
    Number((getComputedStyle(wrapper.value).paddingLeft).replace('px', ''))
)

function setCustomClass(e) {
  Object.keys(e).forEach(key => customClass[key] = e[key])
}
...
function checkArrowVisibility() {
  if (slider.scrolledByTouch) {
    ...
    slider.isHiddenArrow = 
      slider.arrowPosition < -(slider.listOffsetWidth - document.body.offsetWidth) 
      ? true : false
  }
}
...
function handleArrowPosition() {
  if(!slider.scrolledByTouch) 
    slider.arrowPosition = 
      -wrapper.value.scrollLeft + slider.wrapperPaddingLeft
  checkArrowVisibility()
}
...
<template>
  <div 
    ...
    <div 
      ...
      <div 
        ref="arrowBox" 
        :style="{ 'right': `${slider.arrowPosition}px` }"
        :class="customClass.arrowBox"
        class="absolute right-6 flex items-center">
        <a 
          @click="slideToRight"
          :class="customClass.arrowLink"
          class="z-20 flex items-center justify-center bg-slate-50/50 rounded-full">
          <svg 
